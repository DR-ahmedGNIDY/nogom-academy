...
import 'package:basketball_academy/features/attendance/utils/barcode_scan_result.dart';
import 'package:basketball_academy/features/attendance/utils/barcode_scanner.dart';
...
class _AttendanceScanScreenState extends State<AttendanceScanScreen> {
  // الكاميرا تُستخدم على Android فقط؛ على الويب وWindows نعتمد على إدخال يدوي/قارئ USB.
  final bool _canUseCamera =
  ...
  Future<void> _scanWithCamera() async {
    try {
      final result = await scanAttendanceBarcode();
      // Log exactly what the camera returned so issues are diagnosable.
      debugPrint(
          '[ATTENDANCE] camera scan returned: outcome=${result.outcome} rawContent="${result.rawContent}"');
      if (result.outcome == BarcodeScanOutcome.success &&
          result.rawContent.trim().isNotEmpty) {
        await _handleCode(result.rawContent);
      } else if (result.outcome == BarcodeScanOutcome.error) {
        if (mounted) {
          setState(() => _error = 'تعذّر قراءة الكود، حاول مرة أخرى');
        }
      }
      // BarcodeScanOutcome.cancelled → المستخدم أغلق الماسح، لا شيء.
    } catch (e) {
